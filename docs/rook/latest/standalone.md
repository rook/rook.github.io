...
Rook is also easy to run in virutal machines with `vagrant` using the standalone [Vagrantfile](https://github.com/rook/rook/blob/master/demo/standalone/Vagrantfile).  When instructed, `vagrant` will start up CoreOS virtual machines and launch `rook` via `rkt`.  Note that you can make configuration changes as desired in the provided [cloud config file](https://github.com/rook/rook/blob/master/demo/standalone/cloud-config.yml.in).
